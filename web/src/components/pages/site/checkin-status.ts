...
export function accountHasHealthFailure(
  site: Pick<Site, "enabled">,
  account: Pick<SiteAccount, "enabled" | "last_sync_status" | "last_checkin_status">,
) {
  if (accountIsDisabled(site, account)) return false;
  return account.last_sync_status === "failed" || account.last_checkin_status === "failed";
}
