...
import { Activity, ChevronDown, Clock3, Loader2, Play } from 'lucide-react';
...
function AttemptDetails({ attempt }: { attempt: GroupHealthAttempt }) {
    ...
    if (!hasError) {
        return (
            <Card className="gap-0 rounded-2xl border-border/60 bg-card/80 py-0 shadow-xs transition-colors hover:border-border hover:shadow-sm">
            ...
            <Card className="mb-3 gap-0 rounded-xl border-border/70 bg-background/80 py-0 shadow-none">
                <CardContent className="flex items-center justify-between gap-2 px-3 py-1.5">
                    <DialogTrigger asChild>
                        <button type="button" className="grid min-w-0 flex-1 grid-cols-[auto_auto_minmax(0,1fr)] items-center gap-x-2 gap-y-0.5 text-left cursor-pointer">
                            <span className={cn('row-span-2 size-2 rounded-full self-center', statusDotTone(latest?.status))} />
                            <span className="text-sm font-semibold leading-5 text-foreground">{t('title')}</span>
                            <span className="min-w-0 truncate text-xs leading-5 text-muted-foreground">
                                {lastRunRelative}
                            </span>
                            <span className="col-start-2 col-span-2 flex min-w-0 items-center gap-3 text-xs leading-4 text-muted-foreground font-semibold">
                    ...
                    >
                        {isRunning || isRunningMutation ? <Loader2 className="size-3.5 animate-spin" /> : <Play className="size-3.5" />}
                        {t('run')}
                    </Button>
                </CardContent>
            </Card>

            <DialogContent className="flex h-[min(85vh,42rem)] flex-col overflow-hidden rounded-2xl sm:max-w-2xl text-xs">
                <DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
                    ...
                    <Card className="gap-0 rounded-2xl border-border/60 bg-card/80 py-0 shadow-xs">
                        <CardContent className="p-3">
                            <div className="text-[10px] text-muted-foreground">{t('healthy')}</div>
                    ...
                    <Card className="gap-0 rounded-2xl border-border/60 bg-card/80 py-0 shadow-xs">
                        <CardContent className="p-3">
                            <div className="text-[10px] text-muted-foreground">{t('attempts')}</div>
