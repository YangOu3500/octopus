...
const (
	...
	SettingKeyGroupAutoGenerateAssociationTags    SettingKey = "group_auto_generate_association_tags"    // 模型分组批量生成默认标签归并 JSON
...
func DefaultSettings() []Setting {
	return []Setting{
		...
		{Key: SettingKeyGroupAutoGenerateAssociationTags, Value: "[]"},
...
func (s *Setting) Validate() error {
	...
	case SettingKeyGroupAutoGenerateManualAliases:
		...
		return nil
	case SettingKeyGroupAutoGenerateAssociationTags:
		if strings.TrimSpace(s.Value) == "" {
			return nil
		}
		var tags []GroupAutoGenerateAssociationTag
		if err := json.Unmarshal([]byte(s.Value), &tags); err != nil {
			return fmt.Errorf("setting value must be valid JSON array")
		}
		if len(tags) > 100 {
			return fmt.Errorf("setting value must contain at most 100 association tags")
		}
		for _, tag := range tags {
			if strings.TrimSpace(tag.Label) == "" || strings.TrimSpace(tag.Target) == "" {
				return fmt.Errorf("association tag entries must include label and target")
			}
			aliasCount := 0
			...
			if aliasCount == 0 {
				return fmt.Errorf("association tag entries must include at least one alias")
			}
			if aliasCount > 50 {
