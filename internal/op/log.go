...
func requestAttemptsFromRelayLog(relayLog model.RelayLog, fallbackCreatedAt int64) []model.RequestAttempt {
	...
	for i, attempt := range relayLog.Attempts {
		...
		attempts = append(attempts, model.RequestAttempt{
			...
			QuotaStatus:                sanitizeRelayLogText(attempt.QuotaStatus),
			QuotaReason:                sanitizeRelayLogText(attempt.QuotaReason),
			CapacityStatus:             sanitizeRelayLogText(attempt.CapacityStatus),
			CapacityReason:             sanitizeRelayLogText(attempt.CapacityReason),
			CapacityScope:              sanitizeRelayLogText(attempt.CapacityScope),
			CapacitySource:             sanitizeRelayLogText(attempt.CapacitySource),
			LastObservedAt:             attempt.LastObservedAt,
			ExpiresAt:                  attempt.ExpiresAt,
			ChannelConcurrencyMode:     attempt.ChannelConcurrencyMode,
			ChannelConcurrencyLimit:    attempt.ChannelConcurrencyLimit,
			ChannelConcurrencyWaitMS:   attempt.ChannelConcurrencyWaitMS,
			ChannelConcurrencyAcquired: attempt.ChannelConcurrencyAcquired,
			ChannelConcurrencyTimedOut: attempt.ChannelConcurrencyTimedOut,
